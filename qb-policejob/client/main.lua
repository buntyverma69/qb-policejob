...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()                ----1 v
...
RegisterNetEvent('QBCore:Client:OnPlayerUnload', function()              ------2 v
    TriggerServerEvent('police:server:UpdateBlips')
    TriggerServerEvent("police:server:SetHandcuffStatus", false)
    TriggerServerEvent("police:server:UpdateCurrentCops")
    isHandcuffed = false
...
RegisterNetEvent('QBCore:Client:OnJobUpdate', function(JobInfo)                 ---3 v
    PlayerJob = JobInfo
    TriggerServerEvent("police:server:UpdateBlips")
...
RegisterNetEvent('police:client:UpdateBlips', function(players)                                 ---4 v
...
AddEventHandler('911:client:SendPoliceAlert', function(notifyType, msg, type, blipSettings)
    if PlayerJob.name == 'police' or PlayerJob.name == 'ems' and onDuty then
...
AddEventHandler('police:client:PoliceAlertMessage', function(msg, coords)
    if PlayerJob.name == 'police' or PlayerJob.name == 'ems' and onDuty then
        ...
        SetBlipAsShortRange(blip, false)
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("911 - Suspicious situation ")
        EndTextCommandSetBlipName(blip)
        while transG ~= 0 do
            Wait(180 * 4)
            transG = transG - 1
